...
public static class ClassExtentions
{
    ...
    public static void SetPositionInFrontOf(this Transform caller, Transform target, float distance)
    {
        Quaternion rotacao = Quaternion.LookRotation(target.transform.forward);

        Vector3 posicaoDesejada = target.position + rotacao * Vector3.forward * distance;
